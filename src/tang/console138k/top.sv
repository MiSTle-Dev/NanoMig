...
pll_142m pll_hdmi (
   ...
   // .clkout3(), // mspi_clk),        //  phase shifted
    .init_clk(clk),
    .lock(pll_lock),
    .clkin(clk)
);
...
Gowin_CLKDIV clk_div_5 (
    ...
    .resetn(pll_lock_i),   // input resetn
...
assign jtagseln = !(!pll_lock || bl616_jtagsel);
...
//    if(bl616_jtagsel || !pll_lock || !rom_done || osd_reset || kbd_reset)
    if(!pll_lock || !rom_done || osd_reset || kbd_reset)
        reset_cnt <= 16'hffff;
    else if(reset_cnt != 0)
...
// from flash to sdram
assign mspi_clk = clk_85m; // _shifted; // !clk_85m;   
