...
wire	sys_jtagsel;   
assign jtagseln = !(!pll_lock || boot_button_detected || spi_ext || sys_jtagsel);
...
always @(negedge clk_28m) begin
    if(sys_jtagsel || !pll_lock || !rom_done || !reset_n || osd_reset || kbd_reset)
        reset_cnt <= 16'hffff;
    else if(reset_cnt != 0)
...
always @(posedge clk_85m or negedge mem_ready) begin
    if(!mem_ready || sys_jtagsel) begin
