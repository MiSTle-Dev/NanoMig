	...
	if (|autocfg_card) begin
		if (~cfg_z3) begin
			...
			case (chip_addr[6:1])
				...
				6'b000001: begin
					case ({autocfg_card, fastramcfg_d})
...
`else
						...
						// add another two ZII cards when stealing from SlowRAM
						5'b10011: autocfg_data = 4'b0101;	// 1MB extra stolen from SlowRAM
						default:  autocfg_data = 4'b0100;	// 0.5MB extra stolen from SlowRAM
`endif
					endcase
				end
...
`endif
		if (~cfg_z3) begin
			if (chip_addr[6:1] == 6'b100100) begin // Register 0x48 - config, ZII RAM
				...
				autocfg_card <= 2'b00;
`ifndef ENABLE_RAM32
				if (fastramcfg_d[1:0] == 2'b11) begin
					case (autocfg_card)
						2'b01: autocfg_card <= 2'b10;  // Extra 1MiB of RAM stolen from SlowRAM
						2'b10: autocfg_card <= 2'b11;  // Extra 0.5MiB of RAM stolen from SlowRAM
						default: ;
					endcase
				end
`endif
